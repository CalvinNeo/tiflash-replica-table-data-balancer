...
func TestPick(t *testing.T) {
	...
	if LocationLabelMatch(stores, &store1, []int64{2}) {
		t.Errorf("error")
	}
	if LocationLabelMatch(stores, &store1, []int64{3}) {
		t.Errorf("error")
	}
	if LocationLabelMatch(stores, &store1, []int64{4}) {
		t.Errorf("error")
	}
	if !LocationLabelMatch(stores, &store3, []int64{4}) {
		t.Errorf("error")
	}
}
